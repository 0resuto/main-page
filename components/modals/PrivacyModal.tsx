    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
          ...
          >
            <button onClick={onClose} className="absolute top-6 right-6 p-2 bg-brand-bg/50 rounded-full hover:bg-brand-bg transition-colors text-brand-10">
              <X size={20} />
            </button>
            <h2 className="text-3xl font-bold tracking-tight mb-6">Политика конфиденциальности</h2>
