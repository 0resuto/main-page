...
import { Play, Pause, SkipBack, SkipForward, Music, AlertCircle, Volume2, VolumeX, Shuffle } from "lucide-react";
...
export default function MusicPlayer() {
  ...
  useMotionValueEvent(scrollY, "change", (latest) => {
    ...
    // Скрываем при скролле вниз, показываем при скролле вверх или на самом верху страницы
    if (diff > 5 && latest > 50) {
  ...
  const initAudioContext = () => {
    ...
    if (!audioContextRef.current) {
      ...
      analyserRef.current.fftSize = 128; // Дает 64 полосы частот, мы используем первые 32
      analyserRef.current.smoothingTimeConstant = 0.95; // Увеличено для более плавного падения (было 0.8)
      ...
      if (!sourceRef.current) {
        try {
          ...
          gainNodeRef.current.gain.value = volume; // Устанавливаем текущую громкость
          
          sourceRef.current.connect(analyserRef.current);
          analyserRef.current.connect(gainNodeRef.current);
          gainNodeRef.current.connect(audioContextRef.current.destination);
          
          audioRef.current.volume = 1; // Управляем через GainNode, поэтому сам тег всегда на 100%
        } catch (e) {
          console.error("Audio source connect error:", e);
        }
      }
    }
  ...
  const drawVisualizer = () => {
    ...
    // Коэффициент компенсации спада высоких частот.
    // Чем выше индекс (частота), тем сильнее умножается значение.
    const TILT_FACTOR = 0.6;

    // Для увеличения контраста найдем минимальное и максимальное значение в текущем кадре.
    let min = Infinity;
    let max = 0;
    // Анализируем только те частоты, которые мы отображаем (первые 32).
    for (let i = 0; i < numBars; i++) {
      const v = dataArray[i];
      const multiplier = 1 + (i / numBars) * TILT_FACTOR;
      const tiltedV = v * multiplier;

      if (tiltedV > max) max = tiltedV;
      if (tiltedV < min) min = tiltedV;
    }

    // Предотвращаем деление на ноль, если все значения одинаковые (тишина или постоянный тон).
    ...
    for (let i = 0; i < numBars; i++) {
      ...
      // Нормализуем значение: "растягиваем" диапазон от min до max на весь диапазон от 0 до 1.
      // Это делает визуализацию более динамичной и контрастной.
      const percent = (value - min) / range;
      // Устанавливаем высоту от 4px (минимум) до 32px (пик на треть ниже)
  ...
  useEffect(() => {
    if (playlistContainerRef.current) {
      ...
      const activeItem = container.children[currentTrackIndex] as HTMLElement;
      if (activeItem) {
  ...
  const handlePlaylistScroll = () => {
    ...
    // Добавляем 1px для компенсации возможных проблем с округлением браузером
    setShowBottomShadow(Math.ceil(scrollTop + clientHeight) < scrollHeight - 1);
  };

  useEffect(() => {
    const timer = setTimeout(handlePlaylistScroll, 100);
  ...
  const handleNext = () => {
    ...
    if (isShuffle) {
      ...
      // Предотвращаем повторение одного и того же трека подряд
  ...
  const handleProgressPointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    if (e.buttons === 1 && isDraggingProgress) updateProgressFromEvent(e, false);
  };
  ...
  const updateVolumeFromEvent = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (gainNodeRef.current) {
      ...
      audioRef.current.volume = percent; // Фолбек, если Web Audio API еще не инициализирован
    }
  };
  ...
  const handleVolumePointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    if (e.buttons === 1 && isDraggingVolume) updateVolumeFromEvent(e);
  };
  ...
  if (error) {
    return (
      <div className="max-w-5xl mx-auto w-full p-10 relative isolate rounded-3xl shadow-md border border-brand-10/10 bg-brand-60/20 flex flex-col items-center justify-center text-center">
        ...
        <h3 className="text-xl font-bold text-brand-10 mb-2">Failed to connect to Navidrome</h3>
  ...
  const progressPercent = Math.min(100, Math.max(0, (progress / (duration || currentTrack.duration || 1)) * 100));
    ...
    <>
      ...
      >
        ...
        <div className="relative z-10 p-8 lg:p-10 flex flex-col lg:flex-row items-center gap-8 lg:gap-10">
          ...
          <div className="w-68 h-68 lg:w-60 lg:h-60 shrink-0 bg-brand-60/80 border border-brand-10/10 rounded-2xl shadow-xl flex items-center justify-center text-brand-10/30 relative overflow-hidden group-hover:border-brand-30/50 transition-colors">
            ...
            {/* Гистограмма (Визуализатор) */}
            <div 
              ref={visualizerRef}
              className={`absolute bottom-0 inset-x-0 h-16 flex items-end justify-between gap-[2px] transition-opacity duration-700 bg-gradient-to-t from-brand-bg/80 to-transparent ${isPlaying ? 'opacity-100' : 'opacity-0'}`}
            >
              {Array.from({ length: 32 }).map((_, i) => (
                <div key={i} className="w-full bg-brand-30/50 rounded-t-sm will-change-[height]" style={{ height: '4px' }}></div>
              ))}
            </div>
          </div>

          {/* Информация о треке и управление */}
          <div className="flex-1 w-full text-center lg:text-left flex flex-col justify-center">
            <div className="text-xs font-bold uppercase tracking-widest text-brand-30 mb-2">{currentTrack.album || "Мой плейлист"}</div>
            ...
            <div className="w-full flex flex-col lg:flex-row items-center gap-1.5 lg:gap-4 text-xs font-bold text-brand-10/50 mb-6">
              ...
              >
                <div 
                  className={`absolute top-0 left-0 h-full bg-brand-30 rounded-full ${isDraggingProgress ? '' : 'transition-all duration-100'}`}
                  style={{ width: `${progressPercent}%` }}
                >
                  <div className={`absolute right-0 top-1/2 -translate-y-1/2 w-3.5 h-3.5 lg:w-3 lg:h-3 bg-brand-10 rounded-full shadow-md translate-x-1/2 transition-opacity ${isDraggingProgress ? 'opacity-100' : 'opacity-0 group-hover/progress:opacity-100'}`}></div>
                </div>
              </div>
            ...
            {/* Управление плеером и громкостью */}
            <div className="w-full flex items-center justify-center h-14 lg:h-16">
              ...
              <div className="flex items-center lg:absolute lg:left-1/2 lg:-translate-x-1/2">
                <div className="flex items-center justify-end gap-4 lg:gap-6 w-24 lg:w-32">
                  <button onClick={() => setIsShuffle(!isShuffle)} className={`transition-colors hover:scale-110 active:scale-95 ${isShuffle ? 'text-brand-30' : 'text-brand-10/50 hover:text-brand-10'}`}>
                    <Shuffle size={20} />
                  </button>
                  <button onClick={handlePrev} className="text-brand-10/70 hover:text-brand-10 transition-colors hover:scale-110 active:scale-95">
                ...
                <div className="flex items-center justify-start gap-4 lg:gap-6 w-24 lg:w-32">
                  <button onClick={handleNext} className="text-brand-10/70 hover:text-brand-10 transition-colors hover:scale-110 active:scale-95">
              ...
              <div className="hidden lg:flex items-center gap-3 ml-auto">
                <button onClick={toggleMute} className="text-brand-10/70 hover:text-brand-10 transition-colors hover:scale-110 active:scale-95">
                ...
                >
                  <div 
                    className={`absolute top-0 left-0 h-full bg-brand-30 rounded-full ${isDraggingVolume ? '' : 'transition-all duration-100'}`}
                    style={{ width: `${volume * 100}%` }}
                  >
                    <div className={`absolute right-0 top-1/2 -translate-y-1/2 w-3 h-3 bg-brand-10 rounded-full shadow-md translate-x-1/2 transition-opacity ${isDraggingVolume ? 'opacity-100' : 'opacity-0 group-hover/volume:opacity-100'}`}></div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <audio 
              ...
              onTimeUpdate={() => { if (!isDraggingProgress) setProgress(audioRef.current?.currentTime || 0); }}
              onLoadedMetadata={() => {
          ...
          {playlist.length > 1 && (
            <div className="relative z-10 border-t border-brand-10/10 bg-brand-bg/40 p-6 md:p-8">
              <h4 className="text-sm font-bold uppercase tracking-widest text-brand-10/50 mb-4 px-2">Queue</h4>
              <div className="relative">
                {/* Тени поверх списка */}
                <div className={`absolute inset-0 pointer-events-none z-10 rounded-xl shadow-[inset_0_4px_6px_-4px_rgba(0,0,0,0.1)] transition-opacity duration-300 ${showTopShadow ? 'opacity-100' : 'opacity-0'}`}></div>
                <div className={`absolute inset-0 pointer-events-none z-10 rounded-xl shadow-[inset_0_-4px_6px_-4px_rgba(0,0,0,0.1)] transition-opacity duration-300 ${showBottomShadow ? 'opacity-100' : 'opacity-0'}`}></div>
                <div 
                ...
                >
                  ...
                  <div 
                    key={`${track.id}-${idx}`}
                    onClick={() => { setCurrentTrackIndex(idx); setHasActivatedMiniPlayer(true); setIsPlaying(true); }}
                    className={`flex items-center gap-4 p-2 md:p-3 rounded-xl cursor-pointer transition-all duration-300 group/item ${
                      idx === currentTrackIndex ? 'bg-brand-30/10 shadow-sm' : 'hover:bg-brand-10/5 hover:-translate-y-0.5'
                    }`}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className={`font-bold text-sm md:text-base truncate transition-colors ${idx === currentTrackIndex ? 'text-brand-30' : 'text-brand-10 group-hover/item:text-brand-30'}`}>
                        {track.title}
                      </div>
                      <div className="text-xs md:text-sm text-brand-10/50 truncate font-medium mt-0.5">{track.artist}</div>
                    </div>
      ...
      <AnimatePresence>
        {miniPlayerActive && (
          ...
          >
            {/* Отдельный прогресс-бар для мобилки, который остается видимым */}
            ...
            <div className="relative xl:h-16">
              ...
              <div className="relative xl:flex xl:h-full xl:items-center xl:justify-end">
                <div className={`relative overflow-hidden max-xl:border-t max-xl:border-brand-10/10 max-xl:glass max-xl:backdrop-blur-md max-xl:bg-brand-bg/80 max-xl:shadow-sm max-xl:px-4 max-xl:pb-[calc(env(safe-area-inset-bottom,0px)+0.9rem)] max-xl:pt-3 xl:w-full xl:max-w-[420px] transition-transform duration-300 ease-in-out ${!isMobileMiniPlayerVisible ? 'max-xl:translate-y-full' : 'max-xl:translate-y-0'}`}>
                  <div className="flex items-center gap-2 xl:gap-3 xl:py-2">
                    <div className="flex min-w-0 flex-1 items-center gap-3 py-1 xl:py-0">
                      <div className="relative h-14 w-14 shrink-0 overflow-hidden rounded-md border border-brand-10/10 bg-brand-60/80 xl:h-12 xl:w-12 xl:border-brand-10/10 xl:bg-brand-10/8 transform-gpu">
                        {currentTrack.coverUrl ? (
                          // eslint-disable-next-line @next/next/no-img-element
                          <img src={currentTrack.coverUrl} alt={currentTrack.album} className="h-full w-full object-cover" decoding="async" />
                        ) : (
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="truncate text-[11px] font-bold uppercase tracking-[0.24em] text-brand-30/90 xl:hidden mb-0.5">
                          {currentTrack.album || "Мой плейлист"}
                        </div>
                    ...
                    <div className="flex shrink-0 items-center justify-end gap-1 xl:gap-0.5">
                      <button onClick={handlePrev} className="flex h-9 w-9 items-center justify-center rounded-full text-brand-10/70 transition hover:bg-brand-10/8 hover:text-brand-10 xl:h-8 xl:w-8">
                      ...
                      <button onClick={handleNext} className="flex h-9 w-9 items-center justify-center rounded-full text-brand-10/70 transition hover:bg-brand-10/8 hover:text-brand-10 xl:h-8 xl:w-8">
